...
Bare `deepagents` invocations no longer start an interactive REPL. Install and run [Deep Agents Code](/oss/python/deepagents/code/overview) for interactive coding sessions.
...
Managed Deep Agents projects generated by the CLI use a [LangSmith sandbox](/langsmith/sandboxes) backend by default. Use a sandbox backend when the agent needs an isolated environment for code execution, filesystem work, or long-running tasks.
