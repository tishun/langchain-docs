...
LangSmith Self-Hosted v0.15 brings **Context Hub** for version-controlled, environment-aware management of agent instructions and tools, **reusable evaluators and a library of 30+ evaluator templates** that centralize evaluation across your workspace, **per-example assertions** alongside reference outputs in annotation queues, and downloadable **Insights reports** as PDFs for offline analysis. Several breaking changes are worth reviewing before upgrade: the `agent-bootstrap` script is deprecated, the Agent Builder rename to [Fleet](/langsmith/fleet) may require workload-identity service-account updates, and the `projects:update-retention` permission splits into `projects:increase-trace-tier` and `projects:decrease-trace-tier`.
...
- **Context Hub** — create, version, and promote agent and skill contexts in the [Context Hub](/langsmith/use-the-context-hub). It stores each context as a versioned bundle of agent instructions and tools with commit history and `staging` and `production` environment tags.
