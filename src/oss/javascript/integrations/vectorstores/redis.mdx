...
This guide provides a quick overview for getting started with Redis [vector stores](/oss/integrations/vectorstores). The `@langchain/redis` package provides two implementations: `FluentRedisVectorStore` (recommended, with advanced filtering) and `RedisVectorStore` (legacy).
...
This guide uses [OpenAI embeddings](/oss/integrations/embeddings/openai) as an example. You can use [other supported embeddings models](/oss/integrations/embeddings) instead.
...
- **`FluentRedisVectorStore`** (recommended): New implementation with type-safe fluent filtering API, array-based schema definition, and support for GEO and TIMESTAMP fields
...
If you don't provide a `schema`, `FluentRedisVectorStore` can automatically infer the metadata schema from your documents based on the following rules:
...
While automatic inference is convenient for quick prototyping, explicitly defining your schema is recommended for production use cases to:
- Ensure precise field type control (e.g., distinguishing between TEXT and TAG for strings)
- Enable field-specific options like `sortable`, `weight`, or `caseSensitive`
- Improve query performance with optimized indexing
- Make your schema self-documenting and maintainable
</Note>
...
**FluentRedisVectorStore supports five field types:**
...
- **TIMESTAMP**: Date/time fields stored as Unix epoch timestamps (automatically converted from Date objects)
...
- `type`: Field type - `"text"`, `"tag"`, `"numeric"`, `"geo"`, or `"timestamp"` (required)
...
- `FluentRedisVectorStore` uses `schema` config property instead of `customSchema`
- Filter syntax is different - use fluent API (`Tag`, `Num`, etc.) instead of object-based filters
- GEO and TIMESTAMP field types are only available in `FluentRedisVectorStore`
